...
type TransactionByCategoryProps = {
...
export async function getTransactionsByCategory({
  category,
  limit = 3,
}: TransactionByCategoryProps) {
  return _cachedTransactionsByCategory(category, limit);
}

const _cachedTransactionsByCategory = await cache(
  async (category: TransactionCategory, limit: number) => {
    const { db } = await connectToDatabase();
    const transactions = await db
      .collection<TransactionDocument>("transactions")
      .find({ category })
